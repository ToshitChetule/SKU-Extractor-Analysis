...
export default function ExtractionReviewPage() {
  ...
  useEffect(() => {
  // If Aggregated view is selected OR data missing, load from Neo4j
    ...
    fetch("http://localhost:5000/graph/aggregated")
      ...
      .then((data) => {
        if (data && data.columns && data.rows) {
          console.log("✅ Loaded latest graph data:", data);
          setLocalAggregated(data);
        } else {
          console.warn("⚠️ No data returned from Neo4j.");
        }
      })
  ...
  // ✅ Graph-based Refinement (Backend Integrated)
  const handleRefineAttributes = async () => {
    ...
    if (!selectedAttribute) {
      Swal.fire("Please select an Attribute to refine.", "", "info");
      return;
    }
    ...
    try {
      ...
      const newAttr = updated.attribute || selectedAttribute;
      const newValues = updated.values || [];

      // 🧠 Update aggregated matrix
      const newAggregated = { ...localAggregated };
      const rowIndex = (newAggregated.rows || []).findIndex(
        (r) => r[0] === selectedAttribute
      );

      if (rowIndex !== -1) {
        const newRow = [newAttr, ...newValues];
        while (newRow.length < newAggregated.columns.length) newRow.push("");
        newAggregated.rows[rowIndex] = newRow;
      }

      // setLocalAggregated(newAggregated);

      // Refresh UI from Neo4j so the graph and UI stay in sync
      fetch("http://localhost:5000/graph/aggregated")
        .then(res => res.json())
        .then(data => setLocalAggregated(data))
        .catch(err => console.error("Error refreshing data after refinement:", err));
      ...
      Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            <p><b>Old Attribute:</b> ${selectedAttribute}</p>
            <p><b>New Attribute:</b> ${newAttr}</p>
            <p><b>Updated Values:</b> ${
              newValues.length ? newValues.join(", ") : "No changes"
            }</p>
          </div>
        `,
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
            <textarea
              placeholder="Write a prompt to refine selected attribute (e.g., rename edition to service level)"
              value={prompt}
                  ...
                  : (localAggregated.rows || []).map((row, i) => (
                      ...
                      >
                        <td style={{ textAlign: "center", width: "50px" }}>
                          <input
                            type="checkbox"
                            checked={selectedAttribute === row[0]}
                            onChange={() =>
                              setSelectedAttribute(
                                selectedAttribute === row[0] ? null : row[0]
                              )
                            }
                          />
                        </td>
